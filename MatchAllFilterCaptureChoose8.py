...
def MatchAllCluster(save, maxdist=200, filtparam=2.0, SplitTend = 0.8):
    ...
    (sum(distFromCenterAve4)/len(distFromCenterAve4))*4.0*SplitTend,
    (sum(distFromCenterAve5)/len(distFromCenterAve5))*5.0*SplitTend]
    ...
    #remove clusters that are <= 100 points or all superimposed
    i = 0
    while i < groups:
        if len(segregated[i]) <= 100 or np.isnan(np.std(segregated[i])):
...
if __name__== '__main__':
    ...
    #FrameFind()
    while 1<2:
        MatchAllCluster(0,maxdist=80, filtparam=2.0, SplitTend = 0.8)
        cv2.waitKey(10)
